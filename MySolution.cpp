...
Solution::Solution()
{
    ...
    // HNSW parameters - balanced for 98%+ recall and fast search
    M = 20;                      // More connections for better graph quality
    ef_construction = 300;       // High quality graph structure
    ef_search = 300;             // Optimized for speed while maintaining 98%+ recall
    ml = 1.0 / log(2.0);
    max_level = 0;
...
void Solution::kmeans_clustering()
{
    ...
    for (int c = 1; c < nlist; ++c) {
        ...
        float sum_dist = 0.0f;
        for (float d : min_distances) sum_dist += d;
